...
import org.mindrot.jbcrypt.BCrypt; // <--- added for hashing
import java.sql.*;
...
public class ProfileDAO {
        ...
        try (Connection connection = DBConnection.getInstance().getConnection();
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    profile.setPassword(rs.getString("password")); // hashed password from DB
    ...
    public boolean updateProfile(ProfileManage profile) throws SQLException {
        ...
        try (Connection connection = DBConnection.getInstance().getConnection();
            ...
            // <<< ONLY CHANGE: hash password before saving >>>
            String hashedPassword = BCrypt.hashpw(profile.getPassword(), BCrypt.gensalt(12));

            stmt.setString(1, profile.getFullname());
            stmt.setString(2, profile.getUsername());
            stmt.setString(3, hashedPassword); // save hashed password
